...
JOIN M_Product prod ON l.M_Product_ID = prod.M_Product_ID
...
FROM m_product p
WHERE l.m_product_id = p.m_product_id AND p.isexcludeautodelivery = 'N'))) AND l.qtyordered <> l.qtydelivered AND (l.m_product_id IS NOT NULL OR l.c_charge_id IS NOT NULL) AND NOT (EXISTS ( SELECT iol_1.m_inoutline_id
FROM m_inoutline iol_1
JOIN m_inout io ON iol_1.m_inout_id = io.m_inout_id
