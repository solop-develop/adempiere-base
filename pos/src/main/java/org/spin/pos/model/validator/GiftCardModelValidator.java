...
public class GiftCardModelValidator implements ModelValidator {
	...
	public String modelChange(PO po, int type) throws Exception {
		if (po instanceof MProduct) {
			...
			if (type == TYPE_BEFORE_NEW) {
				...
				product.set_ValueOfColumn("IsGenerateGiftCard", productCategory.get_ValueAsBoolean("IsGenerateGiftCard"));
			}
		}
		if (po instanceof MOrderLine) {
			...
			if (type == TYPE_BEFORE_NEW) {
				...
				if (product != null) {
					orderLine.set_ValueOfColumn("IsGenerateGiftCard", product.get_ValueAsBoolean("IsGenerateGiftCard"));
				}
			}
		}
		return null;
	...
	public String docValidate(PO entity, int timing) {
		if (entity instanceof MOrder) {
			if (timing == TIMING_AFTER_COMPLETE) {
				...
				MTable giftCardTable = MTable.get(order.getCtx(), "ECA14_GiftCard");
				MTable giftCardLineTable = MTable.get(order.getCtx(), "ECA14_GiftCardLine");
				...
				String whereClause = " IsGenerateGiftCard = 'Y' AND C_Order_ID = ?";
	...
	private void createGiftCard(MOrder order, MOrderLine orderLine, BigDecimal qty) {
		...
		giftCard.set_ValueOfColumn("IsPrepayment", false);
		giftCard.saveEx();

		PO giftCardLine = giftCardLineTable.getPO(0, order.get_TrxName());
		giftCardLine.set_ValueOfColumn("Amount", qty.multiply(orderLine.getPriceEntered()));
		...
		giftCardLine.set_ValueOfColumn("ECA14_GiftCard_ID", giftCard.get_ValueAsInt("ECA14_GiftCard_ID"));
