 ...
 *
 */
public class PaymentAppoval implements ModelValidator {

	/** Logger */
	private static CLogger log = CLogger.getCLogger(PaymentAppoval.class);
	/** Client */
	...
	public String modelChange(PO entity, int type) throws Exception {
		if(entity.get_TableName().equals(I_C_Payment.Table_Name)) {
			MPayment payment = (MPayment) entity;
			if(type == TYPE_BEFORE_NEW) {
				PaymentApprovalUtil.setDefaultVerificationFromPointOfSales(payment);
			} else if(type == TYPE_AFTER_NEW) {
				PaymentApprovalUtil.sendPaymentVerificationNotification(payment);
			}
		}
		return null;
	}
	...
	public String docValidate(PO entity, int timing) {
		if(timing == TIMING_BEFORE_COMPLETE) {
			if(entity.get_TableName().equals(I_C_Order.Table_Name)) {
				...
				PaymentApprovalUtil.validateApprovalforPayment(order);
