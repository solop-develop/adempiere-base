...
/** Generated Process for (Create Freight Order From Distribution Order)
 *  @author ADempiere (generated) 
 *  @version Release 3.9.4
 */
public class CreateFreightOrderFromDistributionOrder extends CreateFreightOrderFromDistributionOrderAbstract {
	...
	protected String doIt() throws Exception {
		...
		getSelectionKeys().forEach(key -> {
			MDDOrder order = new MDDOrder(getCtx(), key, get_TrxName());
			...
			.withFreightRate((BigDecimal) order.get_Value("FreightRate"))
			.withFreightId(order.get_ValueAsInt("M_Freight_ID"))
			.withFreightCategoryId(getFreightCategoryId())
			.withBusinessPartnerId(order.getC_BPartner_ID())
			.withLocationFromId(warehouse.getC_Location_ID())
			.withDistributionOrderReferenceId(order.getDD_Order_ID())
			;
			if(order.getC_BPartner_Location_ID() > 0) {
				freightLineBuilder.withLocationToId(order.getC_BPartner_Location().getC_Location_ID());
			}
			if(isOverwriteFreightCostRule()) {
