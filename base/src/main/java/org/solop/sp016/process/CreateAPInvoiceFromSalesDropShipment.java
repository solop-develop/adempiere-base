...
public class CreateAPInvoiceFromSalesDropShipment extends CreateAPInvoiceFromSalesDropShipmentAbstract {
	...
	protected String doIt() throws Exception {
		...
		getSelectionKeys().forEach(salesInvoiceLineId -> {
			int purchaseOrderLineId = getSelectionAsInt(salesInvoiceLineId, "CD_C_OrderLine_ID");
			MInvoiceLine salesInvoiceLine = new MInvoiceLine(getCtx(), salesInvoiceLineId, get_TrxName());
			MInvoice salesInvoice = salesInvoiceLine.getParent();
			...
			MDocType salesInvoiceDocumentType = MDocType.get(getCtx(), salesInvoice.getC_DocType_ID());
			if(salesInvoiceDocumentType.getDocBaseType().equals(MDocType.DOCBASETYPE_ARCreditMemo)) {
				multiplier = multiplier.negate();
			}
			multiplier = multiplier.multiply(documentMultiplier);
			if(invoiceLine == null) {
				...
				qtyInvoiced = salesInvoiceLine.getQtyInvoiced().multiply(multiplier);
				invoiceLine.addDescription(Msg.parseTranslation(getCtx(), "@Created@ @from@ @C_Invoice_ID@ ") + 
						salesInvoiceLine.getParent().getDocumentNo() 
				...
				qtyInvoiced = invoiceLine.getQtyInvoiced().add(salesInvoiceLine.getQtyInvoiced().multiply(multiplier));
