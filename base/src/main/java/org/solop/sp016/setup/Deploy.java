...
public class Deploy implements ISetupDefinition {
	 ...
	 */
	private void createModelValidator(Properties context, String transactionName) {
		X_AD_ModelValidator modelValidator = new Query(context, X_AD_ModelValidator.Table_Name, X_AD_ModelValidator.COLUMNNAME_ModelValidationClass + " = ?", transactionName)
				.setParameters(ConsignedMaterial.class.getName())
				.setClient_ID()
		...
		modelValidator.setName("Consignment Management");
		modelValidator.setEntityType("SP016");
		modelValidator.setDescription(SETUP_DESCRIPTION);
		modelValidator.setSeqNo(200);
		modelValidator.setModelValidationClass(ConsignedMaterial.class.getName());
