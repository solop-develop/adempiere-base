...
import org.solop.process.UpdateStorage;
...
/**
 * Default deploy for Storage Update Schedule
 * @author Yamel Senih, yamel.senih@solopsoftware.com, Solop <a href="http://www.solopsoftware.com">solopsoftware.com</a>
 */
public class CreateStorageUpdateSchedule implements ISetupDefinition {
	...
	public String doIt(Properties context, String transactionName) {
		...
		createStorageSchedule(context, transactionName);
		createSnapshotSchedule(context, transactionName);
	 ...
	 */
	private void createStorageSchedule(Properties context, String transactionName) {

		MScheduler scheduler = new MScheduler(context, 0, transactionName);
		scheduler.setAD_Org_ID(0);
		scheduler.setName("Process Storage Queue");
		scheduler.setDescription("Storage Update from Documents");
	 ...
	 * @param transactionName
	 */
	private void createSnapshotSchedule(Properties context, String transactionName) {

		MScheduler scheduler = new MScheduler(context, 0, transactionName);
		scheduler.setAD_Org_ID(0);
		scheduler.setName("Process Stock and Create Snapshot");
		scheduler.setDescription("Create Snapshot and Update Stock");
		scheduler.setAD_Process_ID(UpdateStorage.getProcessId());
		scheduler.setSupervisor_ID(Env.getAD_User_ID(context));
		scheduler.setFrequencyType(MScheduler.FREQUENCYTYPE_Day);
		scheduler.setFrequency(1);
		scheduler.setKeepLogDays(7);
		scheduler.saveEx();
		MProcess process = MProcess.get(context, UpdateStorage.getProcessId());
		process.getParametersAsList().stream().filter(parameter -> parameter.getColumnName().equals(UpdateStorage.ISCREATESNAPSHOT)).findFirst().ifPresent(parameter -> {
			MSchedulerPara schedulerParameter = new MSchedulerPara(context, 0, transactionName);
			schedulerParameter.setAD_Org_ID(0);
			schedulerParameter.setAD_Scheduler_ID(scheduler.getAD_Scheduler_ID());
			schedulerParameter.setAD_Process_Para_ID(parameter.getAD_Process_Para_ID());
